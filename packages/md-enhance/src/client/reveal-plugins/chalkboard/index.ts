...
import type { ChalkboardConfig, ChalkboardChalk } from "./options";
...
interface ChalkboardState {
  /** Whether drawings is loaded */
  loaded: boolean;
  /** Canvas used to draw */
  drawingCanvas: [any, any];
  /** Pen config */
  pens: [ChalkboardChalk[], ChalkboardChalk[]];
}

interface SlideIndice {
  h: number;
  v: number;
}

const getConfig = (userConfig: ChalkboardConfig): ChalkboardConfig => {
  const config: ChalkboardConfig = {
    ...defaultConfig,
    ...userConfig,
    keyBindings: {
...
function whenLoaded(state: ChalkboardState, callback: () => void) {
  // wait for drawings to be loaded and markdown to be parsed
  if (state.loaded !== null) {
    callback();
  } else {
    console.log("Wait for drawings to be loaded");
    setTimeout(whenLoaded, 500, state, callback);
  }
}

const setupDrawingCanvas = (state: ChalkboardState, id: number) => {
  var container = document.createElement("div");
  container.id = state.drawingCanvas[id].id;
  container.classList.add("overlay");
  container.setAttribute("data-prevent-swipe", "true");
  container.oncontextmenu = function () {
    return false;
  };
  container.style.cursor = state.pens[id][color[id]].cursor;
...
const initChalkboard = (reveal: Reveal) => {
  const state: ChalkboardState = {
    loaded: false,
    drawingCanvas: [{ id: "notescanvas" }, { id: "chalkboard" }],
  };

  let background, pen, pens, draw, color, grid;
  ...
  switch (config.theme) {
    case "whiteboard":
      background = ["rgba(127,127,127,.1)", "./img/whiteboard.png"];
      draw = [drawWithBoardmarker, drawWithBoardmarker];
      state.pens = [config.boardmarkers, config.boardmarkers];
      grid = {
    ...
    default:
      ...
      state.pens = [config.boardmarkers, config.chalks];
      grid = {
  ...
  if (
    state.drawingCanvas &&
    (config.theme || config.background || config.grid)
  ) {
    const canvas = document.getElementById(state.drawingCanvas[1].id);
  ...
  setupDrawingCanvas(state, 0);
  setupDrawingCanvas(state, 1);
  ...
  const slideIndices: SlideIndice = {
    h: 0,
    v: 0,
  };
  ...
  function switchBoard(boardIdx) {
    ...
    // broadcast
    const message = new CustomEvent(config.messageType);
  ...
  if (config.storage) {
    ...
    state.loaded = initStorage(sessionStorage.getItem(config.storage));
  }

  if (!state.loaded && config.src != null) {
  ...
  function loadData(filename: string) {
    ...
    xhr.onload = function () {
      if (xhr.readyState === 4 && xhr.status != 404) {
        state.loaded = initStorage(xhr.responseText);
        updateStorage();
   ...
   */
  function getSlideDuration(indices?: any[]) {
    if (!indices) indices = slideIndices;
    var duration = 0;
  ...
  function resetSlide(force?: boolean) {
    var ok =
      force ||
    ...
    if (ok) {
      ...
      resetSlideDrawings();
      // broadcast
      var message = new CustomEvent(config.messageType);
